...
module HpcMarkupMulti (markup_multi_plugin) where
...
markup_multi_plugin :: Plugin
markup_multi_plugin = Plugin { name = "markup-multi"
                             , usage = "[OPTION] .. [<TIX_FILE> [<TIX_FILE> ..]]"
                             , options = markup_options
                             , summary = "Markup Haskell source using multiple tix inputs"
...
markup_main :: Flags -> [FilePath] -> IO ()
markup_main flags files | not (null files) = do
  let hpcflags1 = flags
  let Flags
  ...
  let MTix mtixMods = mergeMTixs mtixs

  mods <-
     sequence [ genHtmlFromMod dest_dir hpcflags1 mtix theFunTotals invertOutput
              | mtix <- mtixMods
              , allowModule hpcflags1 (mtixModuleName mtix)
              ]
...
markup_main _ []
    = hpcError markup_multi_plugin $ "no .tix file or executable name specified"
...
genHtmlFromMod dest_dir flags mtix theFunTotals invertOutput = do
  let theHsPath = srcDirs flags
  let modName0 = mtixModuleName mtix

  (Mix origFile _ _ tabStop mix') <- readMixWithFlags flags (Left modName0)

  let arr_tix :: Array Int TicksCount
      arr_tix = listArray (0,length (mtixModuleTixs mtix) - 1)
              $ mtixModuleTixs mtix

  let tickedWith :: Int -> TicksCount
      tickedWith n = filter ((>0) . snd) (arr_tix ! n)

      isTicked n = sum (snd <$> tickedWith n) > 0
  ...
  let info = [ (pos,theMarkup)
             ...
             , let binBox = case (isTicked gid,isTicked (gid+1)) of
                     (False,False) -> []
                     (True,False)  -> [TickedOnlyTrue (tickedWith gid)]
                     (False,True)  -> [TickedOnlyFalse (tickedWith gid)]
                     (True,True)   -> []

             , let tickBox =
                     if isTicked gid
                     then [IsTicked  (tickedWith gid)]
                     else [NotTicked]

             , theMarkup <- case boxLabel of
                 ExpBox {}      -> tickBox
                 TopLevelBox {} -> TopLevelDecl theFunTotals (tickedWith gid) : tickBox
                 LocalBox {}    -> tickBox
  ...
  content <- readFileFromPath (hpcError markup_multi_plugin) origFile theHsPath
...
data Markup
     = NotTicked
     | TickedOnlyTrue       TicksCount
     | TickedOnlyFalse      TicksCount
     | IsTicked             TicksCount
     | TopLevelDecl    Bool TicksCount
     deriving (Eq,Show)
...
openTick (IsTicked ticks)  = "<span title=\"" <> showTicksCount ticks <> "\" class=\"istickedoff\">"
openTick (TickedOnlyTrue ticks) = "<span title=\"" <> showTicksCount ticks <> "\" class=\"tickonlytrue\">"
openTick (TickedOnlyFalse ticks) = "<span title=\"" <> showTicksCount ticks <> "\" class=\"tickonlyfalse\">"
openTick (TopLevelDecl False _) = openTopDecl
openTick (TopLevelDecl True ticks) =
  case sum (snd <$> ticks) of
    0  -> "<span title=\"" <> showTicksCount ticks <> "\" class=\"funcount\">-- never entered</span>" ++ openTopDecl
    1  -> "<span title=\"" <> showTicksCount ticks <> "\" class=\"funcount\">-- entered once</span>" ++ openTopDecl
    n0 -> "<span title=\"" <> showTicksCount ticks <> "\" class=\"funcount\">-- entered " ++ showBigNum n0 ++ " times</span>" ++ openTopDecl
  where
    showBigNum n | n <= 9999 = show n
...
readMTix file = do
  mtix <- readTix file
  case mtix of
    Nothing -> hpcError markup_multi_plugin $ "error reading tix file from: " ++ file
